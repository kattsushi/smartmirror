...
Enlace.Event = function (status, id_espejo, cb) {
  ...
  if (status){
      Enlace
        .updateAll(
                    {id_espejo: id_espejo},{screen: status})
          .then(
            function (data) {
            },function (err) {
          });
      Enlace
        .find({ where: {id_espejo: id_espejo}},
        function (err,result) {
          if (err) throw err;
...
Enlace.parameterMirror = function (id_espejo, view, cb) {
  ...
  Enlace
    .find({
            fields: {_id: false,view: false,description: false,
            coment: false,name: false,logo: false},
            where: {id_espejo: id_espejo, view: view}
          },
    function (err,result) {
      if (err) { throw err; }
      ...
      if (result.length>0){
        ...
        cb(
            null,ip,port,timeoff,interval,id_espejo,dir,vscreen,update,
